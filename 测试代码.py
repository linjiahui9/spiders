...
# 从name_function模块(文件)中导入get_formatted_name函数，假设要修改程序(添加功能之类的但要确保不破坏这个函数处理只含名和姓的名字的方式，才能保持原来的判断)让其还能判断中间名，就要修改函数，然后进行测试，这样太繁琐了
"""
...
# 单元测试用于核实函数的某个方面没有问题。测试用例是一组单元测试，它们一道核实函数在各种情况下的行为都符合要求

# 先导入unittest模块，再创建一个继承unittest.TestCase的类，在派生类中编写一系列方法对函数进行不同方面测试
'''
import unittest
from name_function import get_formatted_name    # 导入要测试的函数
class NamesTestCase(unittest.TestCase):         # 创建类来继承unittest模块中的TestCase类进行单元测试
    # 测试name_function.py
...
if __name__ == '__main__':      # 许多测试的框架都会先导入测试文件再运行，导入文件时解析器将在导入的同时执行它。(其实就是main主函数，pycharm中直接输入main回车就行)
    ...
    unittest.main()             # 在这里，调用unittest.main()来运行测试用例，要确保该测试文件也不是被测试的文件...，否则变量__name__的值将不会是'__main__'，因此不会调用这句
                                                                        
'''


# 当get_formatted_name方法改成要能处理中间名时(name_function.py改用第二种方法)，再运行上面测试只有姓和名会出错，不能正确处理
...
class TestAnonymousSurvey(unittest.TestCase):   # 继承unittest.TestCase类。
    ...
    def test_store_single_response(self):   # 一个方法测试作为一组单元测试
        ...
        my_survey=AnonymousSurvey(question)     # 创建实例，跳转到类查看init初始化属性要有question
        my_survey.store_response('English')     # 实例调用方法
        self.assertIn('English',my_survey.responses)    # 字符串匹配类的属性(列表)，看survey.py文件中的类和断言方法的png图片
...
# '''
# 方法setUp()：unittest.TestCase类中包括了setUp方法，setUp()方法可以在其内部只创建一次对象，就能在每个测试方法中都能用setUp方法内部创建的对象，不用重复创建
# setUp方法这样在写派生类中重写，派生类在测试时会优先运行setUp方法，setUp方法一般用作创建实例和设置其属性的，在其他方法中直接使用这些实例
import unittest
from unittest import TestCase
from survey import AnonymousSurvey

class TestAnonymousSurvey(TestCase):
    # 继承unittest.TestCase类。不想这样写，想只写用到模块内部的TestCase类名，要改成from unittest import TestCase,不过最后那里还是要用到unittest模块，结果还是要完整的写...
    # 针对AnonymousSurvey开始测试
    def setUp(self):        # setUp创建一个调查对象和答案列表，这样下面的方法测试作为一组单元测试时，可以不用重复创建(实例)对象和(列表)答案
        # 创建一个调用对象和一组答案，供使用的测试方法使用
        question = "What language did you first learn to speak?"
        self.my_survey = AnonymousSurvey(question)  # 创建实例(整个测试就一次，区别上面的代码)
